<template> <!--登录注册 组件-->
  <main class="pageBg"> <!-- 整体背景，为一张图片 -->
    <h1>吴京豪笔记</h1> <!-- 网页的名字 -->
    <article class="form"> <!-- 登录注册整体表单 -->
      <nav class="page-tabs"><!-- 登录注册切换，带数据绑定的 class 显示对应的样式 -->
        <h2 v-bind:class="{show:loginStyle}" v-on:click="showLogin">登录</h2>
        <h2 v-bind:class="{show:isShowRegister}" v-on:click="showRegister">注册</h2>
      </nav>
      <section class="login" v-show="loginStyle"> <!-- 登录页面，条件渲染，只是切换元素的 display CSS property -->
        ...
        <button v-on:click="onLogin">登录</button><!-- 对应页面表单提交的按钮 -->
      </section>
      <section class="register" v-show="isShowRegister"><!-- 注册页面 -->
        ...
        <button v-on:click="onRegister">注册</button>
      </section>
    </article>
  </main>
</template>
...
const loginStyle = ref(true)  // 是否展示登录页面，默认展示登录页面
console.log(loginStyle.value)
console.log(typeof loginStyle.value)

const registerStyle = ref(false)  // 是否展示注册页面，默认不展示
const login = reactive({  // 登录
  username: '',  // 获取用户输入的用户名
  password: '',  // 获取用户输入密码
  notice: '占位',  // 提示，需要占一定的的空间，不然增加时页面样式会有变动（原来为空的话，突增一行东西有变化）
  isError: false  // 是否出错；为true时，<p>标签会有class="error"，对应的内容会显示
})
const register = reactive({  // 注册
  username: '',
  password: '',
  notice: '占位',
  isError: false
})
const showLogin = () => {  // 展示登录页面，监听对应 <h2> 标签的 click 事件
  loginStyle.value = true
  registerStyle.value = false
}
const showRegister = () => {   // 展示注册页面
  loginStyle.value = false
  registerStyle.value = true
}
// 注册页面，监听 input 事件，判断用户设置用户名 过程中 是否符合要求
const isValidRegisterUsername = () => {
...
const onLogin = () => {
  ...
  if (!/^[\w\u4e00-\u9fa5]{3,15}$/.test(this.login.username)) {
    ...
    this.login.notice = '用户名为3~15个字符，支持 _ 、字母、数字、中文'
    return
  }
...
const onRegister = () => {
  ...
  if (!/^[\w\u4e00-\u9fa5]{3,15}$/.test(this.register.username)) {
    this.register.isError = true
    this.register.notice = '用户名为3~15个字符，支持 _ 、字母、数字、中文'
    return
  }
  if (!/^.{6,16}$/.test(this.register.password)) {
    this.register.isError = true
...
<style lang="less" scoped src="../style/login.less"/>